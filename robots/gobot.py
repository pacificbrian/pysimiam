...
import robots.megabot_ctrl_pb2 as megabot_pb2
...
class GoBot(Robot):
    def ackCtrlSend(self, client, userdata, result):
        #print(f"mqtt sent: {self.ang_velocity}")
        pass
    ...
    def __init__(self, pose, color = 0xFFFFFF):
        ...
        self.mqttCtrl = paho.Client("megabot-supervisor")
        #self.mqttCtrl.on_connect
        #self.mqttCtrl.on_disconnect
        self.mqttCtrl.on_publish = self.ackCtrlSend
        self.mqttCtrl.connect("0.0.0.0", 1883)
    ...
    def set_wheel_speeds(self,*args):
        ...
        #print(f"{self.ang_velocity}")

        # call sparki's Motors function!!
        message = megabot_pb2.megabotCtrl()
        message.fvalue1 = left_ms
        message.fvalue2 = right_ms
        self.mqttCtrl.publish("ctrl/Motors", message.SerializeToString())
